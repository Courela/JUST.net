...
namespace JUST.UnitTests.Gramar
{
    ...
    public class JUSTGrammarTests
    {
        ...
        //[TestCase("#valueof($.test)", "valueof", "$.test", 2)]
        //[TestCase("#valueof(#valueof($.path))", "valueof", "$.menu.popup.menuitem[?(@.value=='Open')].onclick", "OpenDoc()")]
        //[TestCase("#ifcondition(expr1,expr2,truevalue,falsevalue)", null, null, "falsevalue")]
        [TestCase("#ifcondition(#valueof($.menu.host),github,#valueof($.menu.repository),fail)", "valueof", "$.menu.repository", "JUST")]
        ...
        public void ValidateGrammar(string expression, string fn, string path, object result)
        {
            ...
            ParseResult parseResult = JUST.Gramar.Grammar<JsonPathSelectable>.Instance.Parse(
                ...
                (fn, convertParameters, parameters, context) => 
                    ReflectionHelper.Caller<JsonPathSelectable>(null, "JUST.Transformer`1", fn, new object[] { path, this._input, context }, true, context),
