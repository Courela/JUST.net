...
namespace JUST.UnitTests.Arrays
{
    ...
    public class LoopingTests
    {
        ...
        {
            const string input = ExampleInputs.ObjectArray;
        ...
        {
            const string input = ExampleInputs.NestedArrays;
        ...
        public void LoopingAlias()
        {
            ...
            const string transformer = "{ " +
                "\"hello\": {" +
                    "\"#loop($.NestedLoop.Organization.Employee, employee)\": {" +
                        "\"Details\": {" +
                            "\"#loop($.Details, details)\": {" +
                                "\"CurrentCountry\": \"#currentvalueatpath($.Country, details)\"," +
                                "\"OuterName\": \"#currentvalueatpath($.Name, employee)\"," +
                                "\"FirstLevel\": {" +
                                    "\"#loop($.Roles, roles)\": {" +
                                        "\"Employee\": \"#currentvalue(employee)\"," +
                                        "\"Job\": \"#currentvalueatpath($.Job, roles)\"" +
                                    "}" +
                                "}" +
                            "}" +
                        "}" +
        ...
        public void DynamicExpression()
        {
            const string input = "{ \"ontologyElements\": [ { \"id\":\"8b8b9d6e-4574-466b-b2c3-0062ad0642fe\", \"name\":\"Ontology1\",\"description\":\"test1\", \"entityType\":\"Ontology\" }, { \"id\":\"3ac89bbd-0de2-4692-a077-1d5d41efab69\", \"name\":\"MainType1\",\"order\":1, \"entityType\":\"MainType\", \"ontologyId\":\"8b8b9d6e-4574-466b-b2c3-0062ad0642fe\" }, { \"id\":\"97aa4eb2-0515-43d6-ba59-ffd931956b1a\", \"name\":\"SubType1\", \"order\":1, \"entityType\":\"SubType\", \"ontologyId\":\"8b8b9d6e-4574-466b-b2c3-0062ad0642fe\", \"mainTypeId\":\"3ac89bbd-0de2-4692-a077-1d5d41efab69\" } ] }";
            const string transformer = "{" + 
                "\"id\": \"#valueof($.ontologyElements[?/(@.entityType == 'Ontology'/)].id)\"," +
                "\"description\": \"#valueof($.ontologyElements[?/(@.entityType == 'Ontology'/)].description)\"," +
                "\"maintypes\": {" +
                    "\"#loop($.ontologyElements[?/(@.entityType == 'MainType'/)])\" : {" +
                        "\"id\": \"#currentvalueatpath($.id)\"," +
                        "\"name\": \"#currentvalueatpath($.name)\"," +
                        "\"order\": \"#currentvalueatpath($.order)\"," +
                        "\"subTypes\" : {" +
                            "\"#loop(#xconcat($.ontologyElements[?/(@.entityType == 'SubType' && @.ontologyId == ', #currentvalueatpath($.ontologyId),'/)]), #concat(inside,Loop), #concat(ro,ot))\": {" +
                                "\"name\": \"#currentvalueatpath($.name,insideLoop)\" } } } } }";
